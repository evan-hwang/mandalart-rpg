...
- **목표 상태 관리**: 할일/완료 상태 토글
- **자동 완료 시스템**: 세부 목표 완료 시 서브/메인 자동 완료
- **진행률 표시**: 그라데이션 프로그레스 바로 달성률 시각화
- **이모지 선택**: 만다라트별 대표 이모지 지정
- **템플릿**: 건강, 공부, 재테크, 취미 등 미리 정의된 템플릿 제공
- **상태 변경 애니메이션**: 완료 시 바운스 효과
- **공유 기능**: 만다라트를 이미지로 공유
...
- **v1.1.0** - UX & 시각화 (이모지 선택, 상태 변경 애니메이션)
- **v1.0.0** - Core MVP (5x5 그리드, 목표 관리, 공유, 템플릿)
